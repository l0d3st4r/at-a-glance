...
# Ear piece (the pad in front of the ear hole). Kept in its own dict rather than making
# TEAM_COLORS a 3-tuple, so render_page1.py's `primary, _secondary = TEAM_COLORS.get(...)`
...
# The facemask gradient runs across the mask (light on the right, dark on the left), not
# top-to-bottom -- this matches the helmet picker Jason designs in.
MASK_LIGHTEN = 0.30
MASK_DARKEN = 0.35
# The ear piece gets a soft radial highlight rather than a linear ramp.
EAR_LIGHTEN = 0.22
EAR_DARKEN = 0.16
...
def helmet_svg(team, mirrored=False, id_prefix=None):
    ...
    return (
        ...
        f'<stop offset="0" stop-color="{_mix(shell, WHITE, shell_top)}"/>'
        f'<stop offset="1" stop-color="{_mix(shell, BLACK, shell_bottom)}"/>'
        "</linearGradient>"
        f'<radialGradient id="{pid}-ear" cx="0.5" cy="0.5" r="0.5" '
        'gradientTransform="translate(0.5, 0.5) scale(1 2) translate(-0.5, -0.5)">'
        f'<stop offset="0" stop-color="{_mix(ear, WHITE, EAR_LIGHTEN)}"/>'
        f'<stop offset="1" stop-color="{_mix(ear, BLACK, EAR_DARKEN)}"/>'
        "</radialGradient>"
